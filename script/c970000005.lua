...
function s.ritualfil(c)
	return c:IsSetCard(sc) and c:IsRitualMonster()
end

function s.thfilter(c)
	return c:IsFaceup() and c:IsSetCard(sc) and c:IsAbleToHand()
end
function s.thtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_ONFIELD) and chkc:IsControler(tp) and s.thfilter(chkc) and chkc~=e:GetHandler() end
